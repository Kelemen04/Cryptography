...
def validate_and_prepare(data):
    ...
    required = ["block_size_bits", "algorithm", "key_bytes", "mode", "padding"]
...
def encrypt_vigenere(plaintext, key):
    ...
    for i in range(len(plaintext)):
        k = key[i % len(key)] if isinstance(key, (bytes, bytearray)) else key[i]
        ciphertext.append((plaintext[i] + k) % 256)
...
def decrypt_vigenere(ciphertext, key):
    ...
    for i in range(len(ciphertext)):
        k = key[i % len(key)] if isinstance(key, (bytes, bytearray)) else key[i]
        plaintext.append((ciphertext[i] - k) % 256)
